...
func TestWaitAPIServiceReady(t *testing.T) {
	re := require.New(t)
	checkWaitAPIServiceReady(re, true /*isAPIServiceMode*/)
	checkWaitAPIServiceReady(re, false /*isAPIServiceMode*/)
}

func checkWaitAPIServiceReady(re *require.Assertions, isAPIServiceMode bool) {
	var (
		cluster *tests.TestCluster
		err     error
	)
	...
	defer scancel()
	s, cleanup := mcs.StartSingleTSOTestServerWithoutCheck(sctx, re, backendEndpoints, tempurl.Alloc())
	for i := 0; i < 12; i++ {
		select {
