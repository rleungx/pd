...
func (c *Coordinator) GetHotRegionsByType(typ utils.RWType) *statistics.StoreHotPeersInfos {
	...
	storesLoadStats := c.cluster.GetStoresLoadStats()
	stores := c.cluster.GetStores()
	var infos *statistics.StoreHotPeersInfos
	switch typ {
	case utils.Write:
		regionStats := c.cluster.RegionWriteStats()
		infos = statistics.GetHotStatus(stores, storesLoadStats, regionStats, utils.Write, isTraceFlow)
	case utils.Read:
		regionStats := c.cluster.RegionReadStats()
		infos = statistics.GetHotStatus(stores, storesLoadStats, regionStats, utils.Read, isTraceFlow)
	default:
	}
