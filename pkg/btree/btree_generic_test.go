...
func TestBTreeSizeInfo(t *testing.T) {
	...
	for k := 0; k < treeSize; k++ {
		assertEq(t, "get k-th", tr.GetAt(k), k)
	}
	for x := 0; x < treeSize; x++ {
	...
	for k := 0; k < treeSize/3; k++ {
		assertEq(t, "after delete get k-th", tr.GetAt(k), 3*k)
	}
	for x := 0; x < treeSize; x++ {
		y, rk := tr.GetWithIndex(x)
		if x%3 == 0 {
			...
			assertEq(t, "after delete get nil", y, 0)
	...
	for k := 0; k < treeSize/3 && k < 100; k++ {
		assertEq(t, "delete max get k-th", tr.GetAt(k), 3*k)
	}
	for x := 0; x < treeSize && x < 300; x++ {
		y, rk := tr.GetWithIndex(x)
		if x%3 == 0 {
			assertEq(t, "delete max get", y, x)
			assertEq(t, "delete max get rank", rk, x/3)
		} else {
			assertEq(t, "delete max get nil", y, 0)
