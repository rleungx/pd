...
func (s *balanceRegionScheduler) Schedule(cluster opt.Cluster) []*operator.Operator {
	...
	sort.Slice(stores, func(i, j int) bool {
		...
		return stores[i].RegionScore(cluster.GetHighSpaceRatio(), cluster.GetLowSpaceRatio(), float64(iOp)) >
			stores[j].RegionScore(cluster.GetHighSpaceRatio(), cluster.GetLowSpaceRatio(), float64(jOp))
	})
