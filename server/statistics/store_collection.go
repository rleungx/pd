...
func (s *storeStatistics) Observe(store *core.StoreInfo, stats *StoresStats) {
	...
	hotReadWeight := store.GetHotReadWeight()
	hotWriteWeight := store.GetHotWriteWeight()
	storeWriteRateByte, storeReadRateByte := storeFlowStats.GetBytesRate()
	storeStatusGauge.WithLabelValues(storeAddress, id, "store_write_rate_bytes").Set(storeWriteRateByte * hotWriteWeight)
	storeStatusGauge.WithLabelValues(storeAddress, id, "store_read_rate_bytes").Set(storeReadRateByte * hotReadWeight)
	storeWriteRateKey, storeReadRateKey := storeFlowStats.GetKeysWriteRate(), storeFlowStats.GetKeysReadRate()
	storeStatusGauge.WithLabelValues(storeAddress, id, "store_write_rate_keys").Set(storeWriteRateKey * hotWriteWeight)
	storeStatusGauge.WithLabelValues(storeAddress, id, "store_read_rate_keys").Set(storeReadRateKey * hotReadWeight)
