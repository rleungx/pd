...
func (s *BalanceSelector) SelectSource(opt opt.Options, stores []*core.StoreInfo, filters ...filter.Filter) *core.StoreInfo {
	...
	for _, store := range stores {
		...
		if result == nil ||
			result.ResourceScore(s.kind, opt.GetLowSpaceRatio(), 0) <
				store.ResourceScore(s.kind, opt.GetLowSpaceRatio(), 0) {
			result = store
		}
	}
	return result
...
func compareStoreScore(opt opt.Options, storeA *core.StoreInfo, scoreA float64, storeB *core.StoreInfo, scoreB float64) int {
	...
	if storeA.RegionScore(opt.GetLowSpaceRatio(), 0) <
		storeB.RegionScore(opt.GetLowSpaceRatio(), 0) {
		return 1
	}
	if storeA.RegionScore(opt.GetLowSpaceRatio(), 0) >
		storeB.RegionScore(opt.GetLowSpaceRatio(), 0) {
